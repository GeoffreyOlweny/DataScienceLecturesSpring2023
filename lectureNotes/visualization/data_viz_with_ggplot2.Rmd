...
## Data Visualization with `ggplot2`
![](img/ggplot2-cheatsheeta.png)

## Data Visualization with `ggplot2`
![](img/ggplot2-cheatsheetb.png)


## The Components of a Graph
We will construct the following graph for the US murders dataset:
\scriptsize
```{r ggplot-example-plot, echo=FALSE, out.width="95%"}
library(dslabs)
data(murders)
library(ggthemes)
library(ggrepel)
...
```{r ggplot-example-2, out.width="60%"}
murders %>% ggplot()
```
...
```{r ggplot-example-3, out.width="80%"}
...
```{r  ggplot-example-4, out.width="80%"}
p + geom_point(aes(population/10^6, total)) +
...
```{r ggplot-example-5, out.width="60%"}
...
```{r ggplot-example-6, out.width="60%"}
p + geom_point(aes(population/10^6, total), size = 3) +
  geom_text(aes(population/10^6, total, label = abb), 
            nudge_x = 1.5)
```
...
```{r ggplot-example-8, out.width="60%"}
p + geom_point(size = 3) +  
...
\footnotesize
```{r ggplot-example-9, out.width="60%"}
p + geom_point(size = 3) +  
...
\footnotesize
```{r ggplot-example-10, out.width="60%"}
...
```{r ggplot-example-11, out.width="70%"}
...
```{r ggplot-example-12, out.width="70%"}
...
```{r ggplot-example-13, out.width="60%"}
p + geom_point(aes(col=region), size = 3) + 
...
```{r gridExtra-example, warning=FALSE, message=FALSE, fig.height=2, fig.width=4}
library(gridExtra)
p1 <- qplot(x)
p2 <- qplot(x,y)
