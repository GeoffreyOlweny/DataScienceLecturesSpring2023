...
```{r region-freq-barplot, out.width="60%"}
...
```{r state-region-distribution, echo=T, out.width="80%"}
...
```{r height-histogram, echo=T, out.width="60%"}
heights %>% filter(sex=="Male") %>% 
  ggplot(aes(height)) + 
...
\footnotesize
```{r height-histogram-geom, out.width="70%"}
...
```{r example-of-smoothed-density, echo=T, out.width="70%"}
heights %>% filter(sex=="Male") %>% 
...
## Smoothed Density 
```{r smooth-density-2, echo=FALSE, out.width = "100%", warning=F}
hist1 <- heights %>% 
  filter(sex=="Male") %>% 
  ggplot(aes(height)) +
...
```{r area-under-curve, echo=FALSE, out.width="70%"}
...
```{r two-densities-one-plot, echo=T, out.width="70%"}
...
You are now ready for the Exercises 1-10 in the **ggplot2_additional_exercises.pdf** file!
<!--
...
```{r normal-distribution-density, echo=T, out.width="50%"}
mu <- 0; s <- 1; norm_dist <- 
...
\scriptsize
```{r data-and-normal-densities, echo=t, out.width="60%"}
...
```{r  ggplot-qq-dparams, eval=FALSE, out.width="60%"}
...
```{r ggplot-qq-standard-units, eval=FALSE, out.width="60%"}
heights %>% 
  filter(sex=="Male") %>%
  ggplot(aes(sample = scale(height))) + 
  geom_qq() +
...
```{r first-boxplot, echo=FALSE, out.width="60%"}
...
```{r female-male-boxplots-geom, echo=FALSE, out.width="60%"}
...
```{r female-male-boxplots, echo=FALSE, out.width="60%"}
...
## Exercises 
You are now ready for the Exercises 11-25 in the **ggplot2_additional_exercises.pdf** file!
...
Images were not needed for the concepts described in this chapter, but we will use images in Section \@ref(vaccines), so we introduce the two geometries used to create images: __geom_tile__ and __geom_raster__. They behave similarly; to see how they differ, please consult the help file. To create an image in `ggplot1` we need a data frame with the x and y coordinates as well as the values associated with each of these. Here is a data frame.
...
```{r ggplot2-image-new-colors, out.width="60%"}
x %>% ggplot(aes(x, y, fill = z)) + 
  geom_raster() + 
...
```{r qplot-example-1, warning=FALSE, message=FALSE, eval=FALSE, out.width="60%"}
qplot(x)
```
...
```{r qplot-example-2, eval=FALSE, out.width="60%"}
...
```{r qplot-example-3, eval=FALSE, out.width="60%"}
...
```{r qplot-example-4, eval=FALSE, out.width="60%"}
...
```{r qplot-example-5, eval=FALSE, out.width="60%"}
...
```{r qplot-example-6, out.width="60%"}
