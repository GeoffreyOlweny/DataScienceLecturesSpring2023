...
In statistics, we are attempting to find the __best fit__ line that uses your data to estimate the unobserved population-level relationship(s) (slope and intercept) between the independent and dependent variables. 

As defined above, this relationship between parameters $\beta_0$ and $\beta_1$ and the data points is called a **linear regression model**. 
...
## Introduction to Regression
To more formally define our regression model: suppose we observe $n$ data pairs and call them 
$${(x_i, y_i), i = 1, ..., n}.$$ 
...
$$\min_{\beta_0,\beta_1}Q(\beta_0,\beta_1)=\sum_{i=1}^n \epsilon_i^2 = \underset{\beta_0,\beta_1}\inf\left\{\sum_{i=1}^n(y_i-\beta_0-x_i\beta_1)^2\right\}.$$

## Introduction to regression
![](regfigs/rss.png)

## Case study: is height hereditary?
Lets take a look that the dataset from which regression was born:
...
\center
![](regfigs/Francis_Galton2.jpg){ width=30% }
...
```{r scatterplot, fig.height = 3, fig.width = 3, out.width="50%", echo=TRUE, fig.align="center"}
...
\small
```{r father-son-regression, fig.height = 3, fig.width = 3, out.width="45%", fig.align="center",message=FALSE}
galton_heights %>% ggplot(aes(father, son)) +
...
galton_heights %>% lm(son ~ father, data = .) %>% 
    tidy(conf.int = TRUE) %>% filter(term == "father") %>%
    select(estimate, conf.low, conf.high) #%>%
    #ggplot(aes(x = estimate, xmin = conf.low, 
               #xmax = conf.high, y=1)) +
...
* `Gender=female` was selected as the reference level, and `gender=male` results in an increase in $\beta_{male}=5.2$ inches (p-value`<2e-16`)
* The number of children $\beta_children$ is not a significant predictor of child height (p-value=0.0825).
